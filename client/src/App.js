    ...
    return (
        <AppBar position="static">
            <Toolbar>
                ...
                {/* Always Show Home */}
                <Button color="inherit" component={Link} to="/">Search Maps</Button>

                {user ? (
                    // --- LOGGED IN VIEW ---
                    <>
                        {/* Everyone can see their own bookings */}
                        <Button color="inherit" component={Link} to="/my-bookings">My Trips</Button>

                        {/* Only Owners see Station Management */}
                        {isOwner() && (
                            <>
                                <Button color="inherit" sx={{ border: '1px solid white', ml: 1, mr: 1 }} component={Link} to="/dashboard">
                                    Manage Stations
                                </Button>
                        ...
                        <Box sx={{ ml: 2, display: 'flex', alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ mr: 1 }}>{user.username} ({user.role})</Typography>
            ...
            <Router>
                ...
                <Routes>
                    ...
                    <Route path="/dashboard" element={
                        <ProtectedRoute ownerOnly={true}> <OwnerDashboard /> </ProtectedRoute>
                    } />
                    <Route path="/add-point" element={
                        <ProtectedRoute ownerOnly={true}> <AddPoint /> </ProtectedRoute>
                    } />
                    <Route path="/owner-bookings" element={
                        <ProtectedRoute ownerOnly={true}> <OwnerBookings /> </ProtectedRoute>
                    } />
                </Routes>
            </Router>
        </AuthProvider>
